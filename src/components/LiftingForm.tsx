...
export default function LiftingForm({ onContinue, formData }: FormStepProps) {
  const [selectedCapability, setSelectedCapability] = useState(formData?.liftingCapability || '');
  ...
  const handleCapabilitySelection = (capability: string) => {
    setSelectedCapability(capability);
    // Auto-forward after a brief delay for visual feedback
    setTimeout(() => {
      if (onContinue) {
        onContinue({ liftingCapability: capability });
      }
    }, 300);
  };
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="mb-12">
              <h1 className="text-2xl font-medium text-gray-900 text-left font-outfit leading-tight">
                Could you lift a full carry-on into the overhead bin solo?
              </h1>
            </div>
            ...
            <div className="flex-1 space-y-6 pb-20">
              {options.map((option) => (
                <button
                  key={option}
                  type="button"
                  onClick={() => handleCapabilitySelection(option)}
