...
export default function MedicalConditionsForm({ onContinue, formData }: FormStepProps) {
  const [selectedConditions, setSelectedConditions] = useState<string[]>(formData?.medicalConditions || []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedConditions.length > 0 && onContinue) {
      onContinue({ medicalConditions: selectedConditions });
    }
  };
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <span className="text-2xl" role="img" aria-label="condition emoji">
                      {condition.emoji}
                    </span>
